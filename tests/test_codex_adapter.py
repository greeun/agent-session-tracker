...
class TestCommands(_Base):
    ...
    def test_relocate_and_subagents_are_supported(self):
        """Both used to be refused outright; the codex spec now carries the
        capability, so the commands run (this thread simply has no subagent)."""
        rc, out, _ = self._run(tk.cmd_subagents, session_id=SID[:8])
        self.assertEqual(rc, 0)
        self.assertIn("has no subagents", out)
        rc, out, _ = self._run(tk.cmd_relocate, session_id=SID[:8],
                               new_cwd=str(self.root), keep_original=False,
                               force=False, dry_run=True, yes=True)
        self.assertEqual(rc, 0)
        self.assertIn("rewrite in place", out)

    def test_claude_only_capabilities_stay_gated(self):
        """attach / jobs / hooks are Claude Code features with no codex
        counterpart, so require_cap still refuses them."""
        target = tk.find_session(SID[:8])
        for cap in ("attach", "jobs", "hooks"):
            err = io.StringIO()
            with redirect_stderr(err):
                allowed = tk.require_cap(target, cap, cap)
            self.assertFalse(allowed, cap)
            self.assertIn("not supported for codex", err.getvalue())
