...
class _CountingParse:
    ...
    이 계측은 **순차 인덱싱 경로에서만** 유효하다. 콜드 로드가 프로세스 풀로
    넘어가면 파싱이 자식 프로세스에서 수행되므로, 부모에 설치한 이 래퍼의
    카운터는 증가하지 않는다. 그래서 아래 테스트들은 `AST_JOBS=1` 로 순차
    경로를 고정한다. 병렬 경로가 같은 결과를 내는지는
    test_index_parallel.py 가 담당한다."""
...
class TestIndexScale(unittest.TestCase):
    def setUp(self):
        # 파싱 횟수를 부모 프로세스에서 세려면 순차 경로여야 한다(_CountingParse 참고).
        self._jobs = os.environ.get("AST_JOBS")
        os.environ["AST_JOBS"] = "1"
