...
def _sm(sid, *, last=None, msgs=0, cwd="", msg=""):
    return tk.SessionMeta(session_id=sid, path=Path(f"/x/{sid}.jsonl"),
                          cwd=cwd, last_ts=last, msg_count=msgs, first_user_msg=msg)
...
class TestSortSessions(unittest.TestCase):
    def setUp(self):
        ...
        self.a = _sm("a", last=_ts(5), msgs=10, cwd="~/zeta", msg="Zebra task")
        self.b = _sm("b", last=_ts(3), msgs=30, cwd="~/alpha", msg="apple task")
        self.c = _sm("c", last=_ts(4), msgs=20, cwd="~/mid", msg="Mango task")
    ...
    def test_message_alphabetical_case_insensitive(self):
        # "apple task" < "Mango task" < "Zebra task" once lowercased
        self.assertEqual(self.ids("message"), ["b", "c", "a"])

    def test_message_reverse_descending(self):
        self.assertEqual(self.ids("message", reverse=True), ["a", "c", "b"])
...
class TestSortDefaults(unittest.TestCase):
    def test_keys_and_natural_directions(self):
        ...
        # MSGS → MESSAGE → PROJECT), so `s` walks left-to-right across the columns.
        self.assertEqual(tk.SORT_KEYS, ("status", "time", "msgs", "message", "project"))
        self.assertEqual(tk._SORT_DEFAULT_DESC,
                         {"time": True, "status": False, "msgs": True,
                          "message": False, "project": False})
...
class TestSortPersistence(unittest.TestCase):
    ...
    def test_roundtrip(self):
        ...
        tk.save_sort("message", True)
        self.assertEqual(tk.load_sort(), ("message", True))
