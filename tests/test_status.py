...
class TestHookInstall(unittest.TestCase):
    def test_specs_cover_events(self):
        ...
        self.assertIn(("ast prompt-hook", 25), specs["UserPromptSubmit"])
        self.assertIn(("ast status-hook", 10), specs["UserPromptSubmit"])
        for ev in ("Notification", "PermissionRequest", "Stop", "SessionEnd"):
            self.assertIn(("ast status-hook", 10), specs[ev])

    def test_is_our_hook_cmd(self):
        self.assertTrue(tracker._is_our_hook_cmd("ast prompt-hook"))
        self.assertTrue(tracker._is_our_hook_cmd("/path/ast status-hook"))
        # cst의 훅은 남의 것이다 — 두 도구를 나란히 쓰는 설정에서 uninstall-hook
        # 이 cst 항목까지 지워 버리면 안 된다.
        self.assertFalse(tracker._is_our_hook_cmd("cst prompt-hook"))
        self.assertFalse(tracker._is_our_hook_cmd("python cst-done.py"))
    ...
    def test_install_then_uninstall_roundtrip(self):
        ...
        self.assertIn("ast prompt-hook", cmds)
        self.assertIn("ast status-hook", cmds)
        ...
        self.assertNotIn("ast prompt-hook", cmds2)
        self.assertNotIn("ast status-hook", cmds2)
...
class TestVersionString(unittest.TestCase):
    ...
    def test_version_is_locked(self):
        self.assertEqual(tracker.__version__, "1.4.1")
